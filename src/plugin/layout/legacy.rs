//! Legacy layout registration (deprecated `IHyprLayout` API).
//!
//! Hyprland v0.54 replaced `IHyprLayout` with separate `ITiledAlgorithm` and
//! `IFloatingAlgorithm` interfaces. This module exists so plugins can still
//! target pre-v0.54 Hyprland. New plugins should use [`super::tiled`] or
//! [`super::floating`] instead.
...
use super::common::{Direction, RectCorner};
...
// SAFETY: Opaque pointer managed by Hyprland's single-threaded event loop.
...
/// Trait for custom window layouts (legacy `IHyprLayout` API).
///
/// Maps to the pure virtual methods of `IHyprLayout` in C++.
/// **Deprecated in v0.54+** — prefer [`super::tiled::TiledAlgorithm`] or
/// [`super::floating::FloatingAlgorithm`].
...
// `*mut c_void`. Double-boxing gives us a thin `*mut LayoutData` for the C FFI boundary.
...
/// Allocate via malloc so the C++ bridge can free with std::free.
...
// Trampolines exist because C++ virtual dispatch can't call Rust trait methods directly.
// The C++ RustLayoutBridge stores an opaque pointer + vtable; these functions let it
// re-enter Rust without needing C++ name mangling or a stable Rust ABI.
...
/// Register a custom window layout (legacy API).
///
/// **Deprecated in v0.54+** — use [`super::tiled::register_tiled_algo`] or
/// [`super::floating::register_floating_algo`] instead.
///
/// # Errors
///
...
pub fn register_layout(
    ...
    if bridge_ptr.is_null() {
        // C++ bridge destructor already called vtable.drop — no Rust-side cleanup needed.
...
/// Unregister a custom layout (legacy API).
///
/// **Deprecated in v0.54+** — use [`super::remove_algo`] instead.
///
/// # Errors
///
/// Returns [`HyprError::NullHandle`] if the plugin handle is null.
